...
# Perf bias
m1 = lm(perfBias ~ Observer*Task, data = s1.wide) %>% 
  summary()

# PDP control
m2 = lm(MeanC ~ Observer*Task, data = s1.wide) %>% 
  summary()

# PDP auto
m3 = lm(AResid ~ Observer*Task, data = s1.wide) %>% 
  summary()

# Error-related distress
m4 = lm(ErrDistress ~ Observer*Task, data = s1.wide) %>% 
  summary()
...
# Perf bias
m5 = lm(perfBias ~ Observer*Task, data = s2.wide) %>% 
  summary()

# PDP control
m6 = lm(MeanC ~ Observer*Task, data = s2.wide) %>% 
  summary()

# PDP auto
m7 = lm(AResid ~ Observer*Task, data = s2.wide) %>% 
  summary()

# Anxiety composite
m8 = lm(Anx_composite ~ Observer*Task, data = s2.wide) %>% 
  summary()
...
``` {r}
kable(m7$coefficients)
```

**Anxiety composite:**
  
``` {r}
kable(m8$coefficients)
```
